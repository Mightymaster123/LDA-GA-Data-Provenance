...
public class TopicModelling {
	...
	public void LDA(int numberOfTopics, int numberOfIterations, boolean topicFile, boolean original_version) throws IOException {
		 ...
		 * the stopwords file that is to be used is to be set in as the file value here.
		 */
		FileInputStream fileInputStream = new FileInputStream(new File(MalletInput.INPUT_FILE_NAME));
		InputStreamReader inputStreamReader = new InputStreamReader(fileInputStream, MalletInput.INPUT_FILE_CHARSET);
		try {
			instances.addThruPipe(new CsvIterator(inputStreamReader, Pattern.compile("^(\\S*)[\\s,]*(\\S*)[\\s,]*(.*)$"), 3, 2, 1)); // data, label, name fields
		...
		/*
		 * StringBuilder topicZeroText = new StringBuilder(); Iterator<IDSorter>
		 ...
		 * testProbabilities = inferencer.getSampledDistribution(testing.get(0), 10, 1,
		 * 5); System.out.println("0\t" + testProbabilities[0]);
		 */
	}
